...
from manifolds import euclidean,lorentz,poincare 
...
class SetIntersection(nn.Module):
    """
    Layer for Intersection between box/vec objects

    Attributes
    ----------
    manifold : euclidean, lorentz or poincare 
    Other parameters taken from https://github.com/hyren/query2box
    """
    def __init__(self, manifold, mode_dims, expand_dims, agg_func=torch.min):
        super(SetIntersection, self).__init__()
        self.manifold = manifold
        self.agg_func = agg_func
        self.pre_mats = ManifoldParameter(torch.FloatTensor(expand_dims, mode_dims), manifold=self.manifold)
        ...
        self.pre_mats_im = ManifoldParameter(torch.FloatTensor(expand_dims, mode_dims), manifold=self.manifold)
        nn.init.xavier_uniform(self.pre_mats_im)
        self.register_parameter("premat_im", self.pre_mats_im)
        self.post_mats_im = ManifoldParameter(torch.FloatTensor(mode_dims, expand_dims), manifold=self.manifold)
...
class CenterSet(nn.Module):
    """
    Layer for setting Center based on vec/box setting
    ...
    Other parameters taken from https://github.com/hyren/query2box
    """
    def __init__(self, manifold, mode_dims, expand_dims, center_use_offset, agg_func=torch.min, bn='no', nat=1, name='Real_center'):
...
class MeanSet(nn.Module):
    """
    Layer for Mean aggregated Centers
...
class MinSet(nn.Module):
    """
    Layer for Minimum over Offsets in Intersection
...
class OffsetSet(nn.Module):
    ...
    Other parameters taken from https://github.com/hyren/query2box
    """
    def __init__(self, manifold, mode_dims, expand_dims, offset_use_center, agg_func=torch.min, name='Real_offset'):
...
class InductiveOffsetSet(nn.Module):
    ...
    Other parameters taken from https://github.com/hyren/query2box
    """
    def __init__(self, manifold, mode_dims, expand_dims, offset_use_center, off_reg, agg_func=torch.min, name='Real_offset'):
        super(InductiveOffsetSet, self).__init__()
        self.manifold = manifold
        self.offset_use_center = offset_use_center
        self.agg_func = agg_func
        self.off_reg = off_reg
        self.OffsetSet_Module = OffsetSet(self.manifold, mode_dims, expand_dims, offset_use_center, self.agg_func)
...
class AttentionSet(nn.Module):
    ...
    Other parameters taken from https://github.com/hyren/query2box
    """
    def __init__(self, manifold, mode_dims, expand_dims, center_use_offset, att_reg=0., att_tem=1., att_type="whole", bn='no', nat=1, name="Real"):
        ...
        self.Attention_module = Attention(self.manifold, mode_dims, expand_dims, center_use_offset, att_type=att_type, bn=bn, nat=nat)
...
class Query2Manifold(nn.Module):
    """
    Main model Query2Manifold

    Test and Metric/Ranking computation steps have been taken from https://github.com/hyren/query2box 

    Attributes
    ----------
    model_name : string to store in the logs location
    nentity : Number of entities in the dataset
    nrelation : Number of relations in the dataset
    hidden_dim : Hyper-parameter to set dimensions of the vec/box
    gamma : Fixed scalar margin
    writer : Tensorboard Summary Writer
    geo : vec/box
    cen :regularizer for centers
    offset_deepsets : inductive or vanilla or min
    center_deepsets : inductive or vanilla or min
    offset_use_center : calculate attention over both center and offset or just offset
    center_use_offset : calculate attention over both center and offset or just offset
    att_reg : attention regularizer
    off_reg : offset regularizer
    att_tem : scaling attention
    euo : if True all entities have different offsets else all entities share offsets
    gamma2 : Fixed scalar margin for leaf entities
    bn : Batch Normalization (before, after or no)
    nat : Number of attention heads
    activation : Activation function
    manifold : Manifold (poincare, lorentz, euclidean)
    curvature : Initial curvature 
    trainable_curvature : Boolean indicator (True make curvature trainable, False static curvature)
    use_semantics : Boolean indicator of whether to use semantic vector embeddings.
    """
